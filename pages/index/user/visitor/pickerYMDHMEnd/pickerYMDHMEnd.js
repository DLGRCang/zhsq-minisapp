...
Component({
  ...
  componentInnerFunction() {
    this.setData({
      ...
      // let dayTime;
      //  if(this.data.ksDate.month.split("")[0] == "0"){
      //   dayTime = this.data.date.split(" ")[0].split("-")[2].split("")[1]
      //  }else{
      //   dayTime = this.data.date.split(" ")[0].split("-")[2]
      //  }
      ...
      // let timeTime;
      //  if(this.data.date.split(" ")[1].split(":")[0].split("")[0] == "0"){
      //   timeTime = this.data.date.split(" ")[1].split(":")[0].split("")[1]
      //  }else{
      //   timeTime = this.data.date.split(" ")[1].split(":")[0]
      //  }
      ...
      // let divisionTime;
      //  if(this.data.date.split(" ")[1].split(":")[1].split("")[0] == "0"){
      //   divisionTime = this.data.date.split(" ")[1].split(":")[1].split("")[1]
      //  }else{
      //   divisionTime = this.data.date.split(" ")[1].split(":")[1]
      //  }
       //console.log(divisionTime)
      let division = [];
      for (let i = 0; i <= 59; i++) {
        if (i < 10) {
      ...
      this.setData({
        pickerArray,
        pickerIndex: [0, date.getMonth(), date.getDate() - 1, date.getHours()-1, date.getMinutes()],
        //pickerIndex: [0, 0, 0, 0, 0],
        chooseIndex: [0, date.getMonth(), date.getDate() - 1, date.getHours()-1, date.getMinutes()-1],
    ...
    pickerChange: function (e) {
      ...
      var headdata = this.data.ksDate.dateString
      var redData = date.dateString

      var repTime = Date.parse(headdata.replace(/-/g, '/'))
      var repTime1 = Date.parse(redData.replace(/-/g, '/'))

      var twoFoor = 24*60*60*1000

      if(repTime1 == repTime){
        verif.tips("离开时间不能和来访时间相同")
      }else if(repTime1 < repTime){
        verif.tips("离开时间不能小于来访时间")
      }else if(repTime1 > (repTime+twoFoor)){
        verif.tips("离开时间不能超过24小时")
      }else{
        this.triggerEvent('onPickerChange1', date);    
      }
